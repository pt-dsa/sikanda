# Tanya SIKANDA: OpenRouter & Supabase Edge Function Design
...
Karena Anda memiliki kekhawatiran yang sangat valid terkait privasi dan kebocoran data ke pihak ketiga (OpenRouter/Google/Meta), kita akan **membuang pendekatan injeksi data**. AI **TIDAK AKAN PERNAH** menerima, melihat, atau membaca isi database SIKANDA (seperti nama pegawai, NIP, atau aset).
...
4. **Hasil:** Pengguna mendapatkan jawaban yang akurat, dan privasi data SIKANDA 100% terjaga di dalam server kita sendiri.

## 3. Sistem Prompt (Persona Humanis & Eksekusi Intent)
Sistem prompt untuk OpenRouter akan dirancang seperti ini:
> "Kamu adalah otak di balik SIKANDA (Sistem Informasi Kepegawaian dan Aset). Tugasmu HANYA menerjemahkan pertanyaan pengguna ke dalam format JSON 'intent' yang dimengerti sistem kami. Kamu tidak akan melihat data pengguna. Berbicaralah dengan ramah hanya sebagai pembuka jika pengguna hanya menyapa."
