...
import { 
  ...
  Settings, LogOut, Menu, Map, FileText, CalendarClock, Users, Bell, CalendarCheck, UserCog
} from "lucide-react";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Mode pengembangan saja (import.meta.env.DEV). Otomatis HILANG di build produksi.
  const loginDev = () => {
    const sess: AppUser = { email: "dev-admin@local", role: "admin", nip: "", nama: "Administrator (DEV)", is_active: true };
    ...
    <aside className={cn(
      ...
      <div className={cn("flex flex-col items-center justify-center border-b border-gray-100/50 dark:border-gray-800/50 overflow-hidden py-4", desktopOpen ? "px-4" : "md:px-2 md:justify-center px-4")}>
        <div className="flex flex-col items-center gap-2 text-blue-700 dark:text-blue-500 font-bold tracking-tight whitespace-nowrap neuglass rounded-2xl p-3 w-full">
          <img src={`${import.meta.env.BASE_URL}logo_kota_tangerang_selatan.png`} alt="SIKANDA Logo" className="w-10 h-10 object-contain" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
