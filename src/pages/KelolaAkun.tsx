...
import {
  ...
  CheckCircle2, Ban, Pencil, Mail, IdCard, Users as UsersIcon, AlertTriangle,
} from "lucide-react";
...
// ── Modal konfirmasi — menggantikan window.confirm() yang diblokir iframe ──
...
const CONFIRM_CLOSED: ConfirmState = {
  open: false, title: "", message: "", confirmLabel: "Ya",
      ...
      >
        <div className="p-5">
          <div className="flex items-start gap-3 mb-3">
            ...
            <div>
              <h3 className="font-bold text-gray-900 dark:text-white text-base">{state.title}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 whitespace-pre-line">{state.message}</p>
            </div>
          </div>
        </div>
        <div className="px-5 pb-5 flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-bold text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700"
          >
            Batal
          </button>
          <button
            onClick={() => { onClose(); state.onConfirm(); }}
            className={`px-5 py-2 text-sm font-bold text-white rounded-xl shadow-sm ${state.confirmClass}`}
          >
            {state.confirmLabel}
          </button>
        </div>
